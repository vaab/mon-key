...
use eframe::{
    egui,
    egui::{Align2, Color32, CursorIcon, FontId, Key, Pos2, Rect, Rounding, Stroke},
};
...
struct AppState {
    rx: Option<Receiver<Event>>, // bound device channel
    gap_ms: u64,
...
impl AppState {
    ...
    fn refresh_devices(&mut self) {
        ...
        if self.selected_dev_idx.is_none() {
            // Check (immutable) first to avoid holding a &mut self borrow while calling methods on self
            let need_clear = match &self.source {
                InputSource::Evdev(p) => !p.as_os_str().is_empty(),
            };
            if need_clear {
                // Now mutate the path in a separate, short-lived borrow
                if let InputSource::Evdev(p) = &mut self.source {
                    *p = PathBuf::new();
                }
                self.rx = None;
    ...
    fn set_evdev_source(&mut self, idx: usize) {
        ...
        self.cur = None;
        self.last_event = None; // reset capture state
        self.set_listening(true); // auto-enable on selection
    }
    ...
    fn force_finalize_now(&mut self) {
        if let Some(mut s) = self.cur.take() {
            ...
            if self.anim_scale.is_none() {
                ...
                self.anim_scale = Some(ScaleAnim {
                    start: Instant::now(),
                    from,
                    to,
                    dur_ms: 220,
    ...
    fn delete_by_uid(&mut self, uid: u64) {
        ...
        if let Some(pos) = self.sequences.iter().position(|s| s.uid == uid) {
            ...
            if !self.sequences.is_empty() {
                let idx = if pos < self.sequences.len() {
                    pos
                } else {
...
fn measure_max_label_px(ctx: &egui::Context, font: &FontId, labels: &[String]) -> f32 {
    ctx.fonts(|f| {
        labels
            .iter()
            .map(|k| {
                f.layout_no_wrap(k.clone(), font.clone(), Color32::WHITE)
                    .size()
                    .x
            })
            .fold(0.0, f32::max)
...
fn draw_listening_ring(
    ...
    p.circle_stroke(
        ...
        Stroke::new(
            ...
            Color32::from_rgba_unmultiplied(
                ...
                255,
                255,
                ((base_alpha as f32) * alpha_mul).clamp(0.0, 255.0) as u8,
            ),
        ),
    );
    if !listening {
    ...
    let rev_per_sec = 0.15;
    let head = (time_secs * rev_per_sec).fract() * TAU;
    let arc_frac = 0.30;
    let arc_ang = arc_frac * TAU;
    let tail = head - arc_ang;
    let steps: usize = 120;
    ...
    for i in 0..=steps {
        ...
        let s = t * t * (3.0 - 2.0 * t);
        let g = s.powf(1.6);
        let alpha = ((g * 235.0) * alpha_mul).clamp(0.0, 255.0) as u8;
...
fn draw_taps(
    ...
    for tap in &seq.taps {
        ...
        painter.circle_filled(
            Pos2::new(to_x(tap.at, tmax, x0, x1), y),
            dot_r,
...
fn draw_sequence_block(
    ...
    let sel_gap = 10.0;
    let left_gutter =
        (measure_max_label_px(ctx, &key_font, &seq.row_order) + label_pad + sel_w + sel_gap)
            .max(30.0);
    let right_pad = 10.0;
    let top_pad = 22.0;
    let row_h = 28.0;
    ...
    if is_live {
        draw_active_holds(
            &painter, seq, now_vis_ms, tmax, x0, x1, y0, row_h, line_thick,
        );
    }
    ...
    painter.line_segment(
        [Pos2::new(a.x, b.y), Pos2::new(b.x, a.y)],
        Stroke::new(sw, col),
    );
...
impl eframe::App for AppState {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if !self.has_device() && self.listening {
            self.set_listening(false);
        }
        self.ingest();
        ...
        if self.prev_recording != recording_now {
            self.ind_fade = Some(IndicatorFade {
                start: Instant::now(),
                dur_ms: 60,
                to_recording: recording_now,
            });
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_source("device_picker")
                    .selected_text(match (&self.source, self.selected_dev_idx) {
                        (InputSource::Evdev(_), Some(i)) => self
                            .devices
                            .get(i)
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        for (i, d) in self.devices.iter().enumerate() {
                            let label = format!(
                                "{}  [{}:{}]  {}",
                                d.name,
                                d.vendor.unwrap_or(0),
                                d.product.unwrap_or(0),
                                d.path.display()
                            );
                            if ui
                                .selectable_label(self.selected_dev_idx == Some(i), label)
                                .clicked()
                            {
                                pick = Some(i);
                            }
                ...
                if self.has_device() {
                    ...
                    let gap_icon = 8.0_f32;
                    let gap_text_icon = 0.0_f32;
                    ...
                    let space = (ui.available_width()
                        - (hint_w + gap_text_icon + icon_sz + right_pad_ui))
                        .max(0.0);
                    ui.add_space(space);
                    let hint_col = if self.listening {
                    ...
                    let (r_icon, p_icon) =
                        ui.allocate_painter(egui::vec2(icon_sz, icon_sz), egui::Sense::click());
                    ...
                    if dot_alpha > 0.001 {
                        ...
                        let pix_pad = 0.5;
                        let r = size_min * 0.5 - pix_pad;
                        let a = (dot_alpha * 255.0).clamp(0.0, 255.0) as u8;
                        p_icon.circle_filled(
                            r_icon.rect.center(),
                            r,
                            Color32::from_rgba_unmultiplied(220, 50, 50, a),
                        );
                    }
            ...
            if blocks.is_empty() {
                ...
                let label: String = if !self.has_device() {
                    ...
                    let dev_name = self
                        ...
                        .or_else(|| match &self.source {
                            InputSource::Evdev(p) if !p.as_os_str().is_empty() => {
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for (idx, (s, is_live)) in blocks.into_iter().enumerate() {
                            let is_target =
                                !is_live && self.anim_target_uid.map_or(false, |id| id == s.uid);
                            let anim = if is_target {
                                self.anim_scale.as_ref()
                            } else {
                                None
                            };
                            let is_selected = self.selected_uid.map_or(false, |id| id == s.uid);
                            let (resp, del_clicked) = draw_sequence_block(
                                ctx,
                                ui,
                                s,
                                is_live,
                            ...
                            if idx + 1
                                < self.sequences.len() + if self.cur.is_some() { 1 } else { 0 }
                            {
                                let sep_h = 6.0;
...
// ===== main =====
fn main() -> Result<()> {
    // Simplified startup: no CLI parsing for now (defaults only)
    let gap_ms: u64 = 500;
    let source = InputSource::Evdev(PathBuf::new()); // no device yet; pick from UI
    let rx_opt: Option<Receiver<Event>> = None;
